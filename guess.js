...
while (restartGame){
    // Asks user to enter a number to set the upper bound for the random number that will be created (The number to be guessed).
    rangeNum = prompt(`Please enter a maximum number for the range:`);

    // Using parseInt to attempt to convert the user's response into a number value. NOTE: The value returned from a prompt is a string value. Also, if the value cannot be converted the value returned will be NaN (Not a Number).
    rangeNum = parseInt(rangeNum); 

    // Verifies the user's entry for a range number is a number greater than zero. NOTE: NaN has a default boolean value of false. Also, all numbers positive and negative, have a default boolean value of true except for zero which has a default boolean value of false.
    while (!rangeNum || rangeNum < 1){
        rangeNum = prompt(enterNumText);
        rangeNum = parseInt(rangeNum);         
    }

    // Creates the random number (Number to be guessed by the user) using the range number entered by the user. 
    randomNum = Math.floor(Math.random() * rangeNum) + 1;

    // Prompts user to enter a number of attempts allowed. Also attempting to convert their response into a number value. 
    ...
    while (true){
        ...
        // Verifies the user's guess is a number within the range set by the user.
        ...
        if (guess === randomNum){
            ...
            // Checks if user's guess was too low and prompts user to guess again if that's the case.
    ...
    while (true){
        // Checks if the user's response is NO (checks both upper and lowercase N.)
        ...
        // Checks if the user's response is a YES (Upper or lower case Y).
